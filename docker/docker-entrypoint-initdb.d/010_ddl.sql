...
CREATE TABLE IF NOT EXISTS fixtures (
  id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  tournament_id INT REFERENCES tournaments ON DELETE CASCADE,
  home_team INT REFERENCES tournament_teams ON DELETE CASCADE,
  away_team INT REFERENCES tournament_teams ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS league_user_entries (
  id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS entry_picks (
  id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  league_user_id INT REFERENCES league_users ON DELETE CASCADE,
  league_user_entry_id INT REFERENCES league_user_entries ON DELETE CASCADE,
  tournament_team_id INT REFERENCES tournament_teams ON DELETE CASCADE,
  position INT NOT NULL,
  UNIQUE (league_user_entry_id, position),  -- All positions in an entry have to be unique: 1, 2, 3 ... or 8
  UNIQUE (league_user_entry_id, tournament_team_id),  -- No country can be used twice in an entry
